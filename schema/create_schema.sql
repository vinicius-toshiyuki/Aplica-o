...
CREATE TABLE IF NOT EXISTS LISTA (
	...
	PROVA CHAR(1) NOT NULL CHECK (PROVA IN ('S', 'N')),
...
CREATE OR REPLACE VIEW PROBLEMAS_COMPLETOS AS (
	...
	FROM
		PROBLEMA P, LISTA L
	WHERE
		P.LISTA_COD = L.COD AND
		P.MODULO_COD = L.MODULO_COD AND
		P.DISC_COD = L.DISC_COD
);

CREATE OR REPLACE VIEW LING_PROGR_DAS_DISCIPLINAS AS (
		SELECT
			LP.COD,
			LP.NOME,
			LP.COMAND_COMPILA,
			LPD.DISC_COD
		FROM
			LING_PROGR LP, LING_PROGR_DISCI LPD
		WHERE
			LP.COD = LPD.LING_PROGR_COD
);